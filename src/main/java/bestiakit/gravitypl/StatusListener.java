...
public class StatusListener implements Listener {
	...
	public void playerHitedAnotherPlayer(EntityDamageByEntityEvent e) {
		...
		if (takerPlayer.getLastDamageCause() == null)
			return;
	...
	public void playerTeleported(PlayerTeleportEvent e) {
		if (!isAllowedTeleportEffects)
			return;
		Player p = (Player) e.getPlayer();
		if (p.isOp())
			return;
		if (!(e.getCause() == TeleportCause.COMMAND || e.getCause() == TeleportCause.PLUGIN
				|| e.getCause() == TeleportCause.END_PORTAL || e.getCause() == TeleportCause.NETHER_PORTAL))
			return;
	...
	public void playerGivesToAnotherPlayer(EntityPickupItemEvent e) {
		...
		if (!(pickedItem.getType() instanceof Material))
			return;
